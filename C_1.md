...
<div align="center">
   <img src="images/Deep_Learning_ND_P1_C_1_01.png" alt="AI, ML, and DL Relationship" width="600" height=auto/>
...
<div align="center">
   <img src="images/Deep_Learning_ND_P1_C_1_02.png" alt="Board" width="450" height=auto/>
...
- The output layer typically has a single neuron (for single-value prediction)
...
<div align="center">
   <img src="images/Deep_Learning_ND_P1_C_1_03.png" width="400" height=auto/>
...
This is precisely what we want for binary classification. We can think of the exponents as "selectors" that
...
<div align="center">
   <img src="images/Deep_Learning_ND_P1_C_1_04.png" width="250" height=auto/>
...
I'd be happy to explain this neural network calculation step by step. Let me break down exactly what's happening with
this simple network that has two inputs, one hidden unit, and one output unit.

First, let's clarify the values:

- The 0.1 and -0.2 are the weights connecting the input units to the hidden unit (often initialized randomly)
- The 0.1 in the output calculation is the weight connecting the hidden unit to the output unit
- 0.4 and 0.3 are our input values (the features of our data point)
...
###### Step 1: Calculate Input to Hidden Unit

$h = 0.1 × 0.4 - 0.2 × 0.3 = -0.02$

Here we're calculating the weighted sum that goes into the hidden unit:

- The first input (0.4) is multiplied by its weight (0.1): $0.1 × 0.4 = 0.04$
- The second input (0.3) is multiplied by its weight (-0.2): $-0.2 × 0.3 = -0.06$
- We sum these values: $0.04 + (-0.06) = -0.02$

This is exactly what the linear part of a neuron does: multiply each input by its corresponding weight and add them all
together. (Note that for simplicity, this example doesn't include bias terms, but real networks typically would.)

###### Step 2: Apply Activation Function

$a = σ(-0.02) = 0.495$

Now we apply the sigmoid activation function to the weighted sum: $σ(z) = \frac{1}{1+e^{-z}}$

For z = -0.02: $σ(-0.02) = \frac{1}{1+e^{0.02}} = \frac{1}{1.0202} ≈ 0.495$

The sigmoid function squashes the input to a value between 0 and 1, which can be interpreted as a probability.

###### Step 3: Calculate Final Output

$\hat{y} = σ(0.1 × 0.495) = 0.512$

Now we pass the activation from the hidden unit to the output unit:

- The hidden unit's activation (0.495) is multiplied by its weight to the output (0.1): $0.1 × 0.495 = 0.0495$
- We apply the sigmoid activation again: $σ(0.0495) = \frac{1}{1+e^{-0.0495}} ≈ 0.512$

This gives us our final prediction of 0.512 (about a 51.2% probability of the positive class).

###### Understanding the Backward Pass (Backpropagation)

Now for the backward pass, we compute how much each weight contributed to the error, so we can update the weights to
reduce the error.

###### Step 1: Calculate Output Error Term

$δ^o = (1 - 0.512) × 0.512 × (1 - 0.512) = 0.122$

This calculation is a bit more involved:

- $(1 - 0.512)$ is the error: target (1) minus prediction (0.512) = 0.488
- $0.512 × (1 - 0.512)$ is the derivative of the sigmoid function at the output: $σ(z)(1-σ(z)) = 0.512 × 0.488 = 0.25$
- Multiplying these: $0.488 × 0.25 = 0.122$

This represents how much the output node needs to adjust.

###### Step 2: Calculate Hidden Error Term

$δ^h = 0.1 × 0.122 × 0.495 × (1 - 0.495) = 0.003$

Here we're calculating how much the hidden unit contributed to the error:

- 0.1 is the weight connecting the hidden unit to the output
- 0.122 is the output error term we just calculated
- $0.495 × (1 - 0.495)$ is the derivative of the sigmoid at the hidden unit: $≈ 0.25$
- Multiplying these: $0.1 × 0.122 × 0.25 = 0.003$

This shows how the error propagates backwards through the network, getting smaller at each step (which illustrates the
vanishing gradient problem mentioned in the text).

###### Step 3: Update Weights

With learning rate η = 0.5:

1. Hidden to output weight: $ΔW = 0.5 × 0.122 × 0.495 = 0.0302$

    This means the new weight would be: $0.1 + 0.0302 = 0.1302$

2. Input to hidden weights: $Δw_1 = 0.5 × 0.003 × 0.4 = 0.0006$ $Δw_2 = 0.5 × 0.003 × 0.3 = 0.00045$

    This means the new weights would be:

    - First weight: $0.1 + 0.0006 = 0.1006$
    - Second weight: $-0.2 + 0.00045 = -0.19955$

Notice how much smaller the updates to the input-hidden weights are compared to the hidden-output weight. This is
exactly the vanishing gradient problem in action! As we go further back in the network, the error signal (gradient)
becomes increasingly small, making it difficult for early layers to learn effectively in deep networks.

This simple example beautifully illustrates both the forward and backward passes in neural networks and highlights why
alternatives to sigmoid activation (like ReLU) became necessary for training deeper networks effectively.

###### Why We Use Derivatives in Backpropagation

During backpropagation, we need to find how much each weight contributed to the error so we can update the weights
appropriately. This requires calculating the gradient of the error with respect to each weight.

The chain rule from calculus allows us to decompose this gradient into parts. For the sigmoid activation function, we
need its derivative as part of this calculation.
...
This means if we already know the sigmoid output, we can easily calculate its derivative.

###### In Our Example

In the output error term calculation:

$$\delta^o = (1 - 0.512) \times 0.512 \times (1 - 0.512)$$

Let's break this down:

- $(1 - 0.512)$ represents $(y - \hat{y})$, the difference between target and prediction
- $0.512 \times (1 - 0.512)$ is $\sigma'(z) = \sigma(z)(1-\sigma(z))$, the derivative of the sigmoid at the output

The full calculation according to the chain rule is:

$$\delta^o = \frac{\partial E}{\partial \hat{y}} \times \frac{\partial \hat{y}}{\partial z} = (y - \hat{y}) \times \sigma'(z)$$

Where:

- $\frac{\partial E}{\partial \hat{y}}$ is the derivative of the error function with respect to the output
- $\frac{\partial \hat{y}}{\partial z}$ is the derivative of the activation function

This gives us the rate of change of the error with respect to the input to the output neuron, which we need to determine
...
The gradient is a vector of partial derivatives that points in the direction of the steepest increase of a function. For
a function $f$ with multiple variables, the gradient is denoted as $\nabla f$.

For a function $f(x_1, x_2, ..., x_n)$ with $n$ variables, the gradient is:

$$\nabla f = \begin{pmatrix} \frac{\partial f}{\partial x_1} \ \frac{\partial f}{\partial x_2} \ \vdots \ \frac{\partial f}{\partial x_n} \end{pmatrix}$$

Each component $\frac{\partial f}{\partial x_i}$ represents how much the function $f$ changes when we make a small
change to the variable $x_i$.

In neural networks, we use gradients to minimize the error function. If we consider our error function $E$ as a function
of the weights $w$, then the gradient $\nabla E$ tells us how the error changes with respect to each weight.

For a weight $w_{ij}$, the corresponding element of the gradient is:

$$\frac{\partial E}{\partial w_{ij}}$$

This tells us how much the error would change if we made a tiny adjustment to that particular weight.

The fundamental idea of gradient descent is to update each weight in the opposite direction of its gradient component:

$$w_{ij}^{\text{new}} = w_{ij}^{\text{old}} - \eta \frac{\partial E}{\partial w_{ij}}$$

Where $\eta$ is the learning rate. This update rule guarantees (for small enough $\eta$) that we're moving in a
direction that reduces the error.

In backpropagation, we efficiently calculate these gradients by applying the chain rule of calculus, which is why we
need derivatives of activation functions like sigmoid.

For a simple network like in our example, the gradient components are:

$$\frac{\partial E}{\partial W} = -\delta^o \cdot a$$

for the hidden-to-output weight, and:

$$\frac{\partial E}{\partial w_i} = -\delta^h \cdot x_i$$

for the input-to-hidden weights, where $\delta^o$ and $\delta^h$ are the error terms we calculated.

The negative signs appear because we want to move in the direction that decreases the error, which is opposite to the
gradient direction (which points toward increasing values).
...
<div align="center">
   <img src="images/Deep_Learning_ND_P1_C_1_05.png" width="600" height=auto/>
...
<div align="center">
   <img src="images/Deep_Learning_ND_P1_C_1_06.png" width="600" height=auto/>
...
1. It is not differentiable at $z = 0$ and has a derivative of 0 everywhere else, making it incompatible with
...
<div align="center">
   <img src="images/Deep_Learning_ND_P1_C_1_07.png" width="600" height=auto/>
...
<div align="center">
   <img src="images/Deep_Learning_ND_P1_C_1_08.png" width="600" height=auto/>
   <p>Figure: MLP Structure</p>
</div>
